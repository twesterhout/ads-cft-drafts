...
template <class... Batch> auto make_g_dd(Func z, Func Q, Expr L, Expr mu, Batch... i) -> Func {
    Var _mu{"mu"}, _nu{"nu"};
    Func output{"g_dd"};
...
template <class... Batch>
auto make_DDg_dddd(Func z, Func Q, Func DQ, Func DDQ, Expr L, Expr mu, Batch... i) -> Func {
...
template <class... Batch>
auto make_Gamma_Udd_ref(Func z, Func Q, Func DQ, Expr L, Expr mu, Batch... i) -> Func {
...
template <class... Batch>
auto make_DGamma_dUdd_ref(Func z, Func Q, Func DQ, Func DDQ, Expr L, Expr mu, Batch... i) -> Func {
...
template <class... Batch> auto make_g_UU(Func z, Func Q, Expr L, Expr mu, Batch... i) -> Func {
    Var _mu{"mu"}, _nu{"nu"};
    Func output{"g_UU"};
    output(i..., _mu, _nu) = cast<double>(0);
...
auto make_Gamma_Udd(Func g_UU, Func Dg_ddd) -> Func {
    ...
    Gamma_Udd(_, lambda, mu, nu) =
        0.5f * sum(g_UU(_, lambda, rho) *
                   (Dg_ddd(_, mu, nu, rho) + Dg_ddd(_, nu, mu, rho) - Dg_ddd(_, rho, mu, nu)));
    return Gamma_Udd;
}
...
auto make_DGamma_dUdd(Func g_UU, Func Dg_dUU, Func Dg_ddd, Func DDg_dddd) -> Func {
    ...
    DGamma_dUdd(_, kappa, lambda, mu, nu) =
        0.5f * sum(Dg_dUU(_, kappa, lambda, rho) *
                       (Dg_ddd(_, mu, nu, rho) + Dg_ddd(_, nu, mu, rho) - Dg_ddd(_, rho, mu, nu)) +
                   g_UU(_, lambda, rho) *
                       (DDg_dddd(_, kappa, mu, nu, rho) + DDg_dddd(_, kappa, nu, mu, rho) -
...
auto make_Xi_U(Func g_UU, Func Gamma_Udd, Func Gamma_Udd_ref) -> Func {
    ...
    Func Xi_U{"Xi_U"};
    auto expr =
        g_UU(_, nu, lambda) * (Gamma_Udd(_, mu, nu, lambda) - Gamma_Udd_ref(_, mu, nu, lambda));
...
auto make_DXi_dd(Func g_dd, Func Dg_ddd, Func Xi_U, Func DXi_dU) -> Func {
    ...
    Func output;
    output(_, lambda, mu) =
        sum(Dg_ddd(_, lambda, mu, nu) * Xi_U(_, nu) + g_dd(_, mu, nu) * DXi_dU(_, lambda, nu));
    return output;
}
...
auto covariant_derivative(Func v_d, Func Dv_dd, Func Gamma_Udd) -> Func {
    ...
    RDom lambda{0, 4, "lambda"};
    Func output;
    output(_, mu, nu) = Dv_dd(_, mu, nu) - sum(Gamma_Udd(_, lambda, nu, mu) * v_d(_, lambda));
    return output;
}
...
auto make_R_Uddd(Func Gamma_Udd, Func DGamma_dUdd) -> Func {
    ...
    output(_, sigma, rho, mu, nu) = DGamma_dUdd(_, mu, sigma, nu, rho) -
                                    DGamma_dUdd(_, nu, sigma, mu, rho) +
...
template <class... Batch> auto make_F_dd(Func z, Func Q, Func DQ, Batch... i) -> Func {
    Var _mu{"mu"}, _nu{"nu"};
    Func output{"F_dd"};
    output(i..., _mu, _nu) = cast<double>(0);
...
template <class... Batch> auto make_DF_ddd(Func z, Func Q, Func DQ, Func DDQ, Batch... i) -> Func {
...
auto make_DF_dUd(Func F_dd, Func DF_ddd, Func g_UU, Func Dg_dUU) -> Func {
    ...
    RDom lambda{0, 4, "lambda"};
    Func output{"DF_dUd"};
    output(_, rho, mu, nu) = sum(Dg_dUU(_, rho, mu, lambda) * F_dd(_, lambda, nu) +
...
auto make_DivF_d(Func F_Ud, Func DF_dUd, Func Gamma_Udd) -> Func {
    ...
    Func output;
    output(_, nu) = sum(rho, DF_dUd(_, rho, rho, nu) +
                                 sum(lambda, Gamma_Udd(_, rho, rho, lambda) * F_Ud(_, lambda, nu)) -
                                 sum(lambda, Gamma_Udd(_, lambda, rho, nu) * F_Ud(_, rho, lambda)));
    return output;
}

auto make_wave_equation(Func Df_d, Func DDf_dd, Func g_UU, Func Dg_dUU, Func Gamma_Udd, Func DV_f)
    -> Func {
    RDom lambda{0, 4, "lambda"}, mu{0, 4, "mu"}, rho{0, 4, "rho"};
    Func output;
    output(_) = sum(rho, sum(mu, Dg_dUU(_, rho, rho, mu) * Df_d(_, mu) +
...
template <class... Batch> auto make_V(Func z, Func Q, Expr L, Batch... i) -> Func {
    Func output;
    FROM_EXPRESSIONS_V(output);
    return output;
}

template <class... Batch> auto make_DV_Phi(Func z, Func Q, Expr L, Batch... i) -> Func {
    Func output;
    FROM_EXPRESSIONS_DV_Phi(output);
    return output;
}

template <class... Batch> auto make_DV_Chi(Func z, Func Q, Expr L, Batch... i) -> Func {
    Func output;
    FROM_EXPRESSIONS_DV_Chi(output);
    return output;
}

template <class... Batch> auto make_DPhi_d(Func z, Func Q, Func DQ, Batch... i) -> Func {
    Var mu{"mu"};
    Func output{"DPhi_d"};
...
template <class... Batch> auto make_DChi_d(Func z, Func Q, Func DQ, Batch... i) -> Func {
    Var mu{"mu"};
    Func output{"DChi_d"};
    output(i..., mu) = cast<double>(0);
...
auto make_G_dd(Func R_dd, Func DPhi_d, Func DChi_d, Func F_dd, Func F_Ud, Func F_UU, Func g_dd,
    ...
    output(_, mu, nu) =
        ...
        (sum(lambda, -F_dd(_, mu, lambda) * F_Ud(_, lambda, nu)) -
         g_dd(_, mu, nu) / 4 * sum(lambda, sum(rho, F_dd(_, lambda, rho) * F_UU(_, lambda, rho))));
    return output;
}
...
class compute_all_generator : public Halide::Generator<compute_all_generator> {
  public:
    ...
    void generate() {
        ...
        auto Gamma_Udd_ref = make_Gamma_Udd_ref(z, Q, DQ, length, chemical_potential, i);
        auto DGamma_dUdd_ref = make_DGamma_dUdd_ref(z, Q, DQ, DDQ, length, chemical_potential, i);
        ...
        auto DDChi_dd = make_DDChi_dd(z, Q, DQ, DDQ, i);
        {
            ...
            EOM_Phi(i, 0) = make_wave_equation(DPhi_d, DDPhi_dd, g_UU, Dg_dUU, Gamma_Udd,
                                               make_DV_Phi(z, Q, length, i))(i);
        }
        {
            EOM_Chi(i, Var{"temp"}) = cast<double>(0);
            EOM_Chi(i, 0) = make_wave_equation(DChi_d, DDChi_dd, g_UU, Dg_dUU, Gamma_Udd,
...
class einstein_equations_generator : public Halide::Generator<einstein_equations_generator> {
  public:
    ...
    void generate() {
        ...
        auto g_dd = make_g_dd(z, Q, length, chemical_potential, i);
        auto g_UU = make_g_UU(z, Q, length, chemical_potential, i);
        auto Dg_ddd = make_Dg_ddd(z, Q, DQ, length, chemical_potential, i);
        auto Dg_dUU = make_Dg_dUU(z, Q, DQ, length, chemical_potential, i);
        auto DDg_dddd = make_DDg_dddd(z, Q, DQ, DDQ, length, chemical_potential, i);
        auto Gamma_Udd_ref = make_Gamma_Udd_ref(z, Q, DQ, length, chemical_potential, i);
        auto DGamma_dUdd_ref = make_DGamma_dUdd_ref(z, Q, DQ, DDQ, length, chemical_potential, i);
        ...
        auto F_dd = make_F_dd(z, Q, DQ, i);
        auto DF_ddd = make_DF_ddd(z, Q, DQ, DDQ, i);
        ...
        auto F_UU = make_F_UU(F_Ud, g_UU);
        auto V = make_V(z, Q, length, i);
        auto G_dd = make_G_dd(R_dd, DPhi_d, DChi_d, F_dd, F_Ud, F_UU, g_dd, V, length);

        output(i, j) = cast<double>(0);
        output(i, 0) = G_dd(i, 0, 0);
        output(i, 1) = G_dd(i, 1, 1);
        output(i, 2) = G_dd(i, 2, 2);
        output(i, 3) = G_dd(i, 3, 3);
        output(i, 4) = G_dd(i, 1, 3);
        output(i, 5) = DivF_d(i, 0);
        output(i, 6) = make_wave_equation(DPhi_d, DDPhi_dd, g_UU, Dg_dUU, Gamma_Udd,
                                          make_DV_Phi(z, Q, length, i))(i);
        output(i, 7) = make_wave_equation(DChi_d, DDChi_dd, g_UU, Dg_dUU, Gamma_Udd,
                                          make_DV_Chi(z, Q, length, i))(i);
    }
};

HALIDE_REGISTER_GENERATOR(compute_all_generator, compute_all_generator)
HALIDE_REGISTER_GENERATOR(einstein_equations_generator, einstein_equations_generator)
