...
import Metric.Petsc.Context
import Metric.Petsc.Types
import UnliftIO.Exception (bracket)
import UnliftIO.Foreign

C.context (C.baseCtx <> C.funCtx <> petscCtx)
...
petscInitialize =
  liftIO $ [CU.exp| void { PetscCallAbort(PETSC_COMM_WORLD, PetscInitializeNoArguments()) } |]
